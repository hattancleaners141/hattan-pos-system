    ...
    const response=await fetch('https://api.openai.com/v1/responses',{
      ...
      body:JSON.stringify({
        ...
        input:[{role:'user',content:[
          {type:'input_text',text:`Read this CleanBase dry-cleaning POS ticket screenshot (${sourceName}). Extract ONLY what is visibly supported. The centered ticket window is the primary source. IMPORTANT: read the customer panel behind the ticket window carefully. Capture the FULL customer name exactly as displayed, even when part of the name is near the left edge. Also capture phone/address into the customers array whenever legible. Prefer the complete customer-panel identity over a partial name inferred from the ticket window. Preserve the ticket number exactly including hyphens. Extract drop-in/created date, due date, status/out date if useful, rack/location, quantity/service/item details, colors/materials, displayed unit or extended prices, discounts in the item/notes text when present, grand total, balance and paid state. For multiple garment lines, combine them into one concise items string without inventing details. If Balance is 0.00, paid=true. If Balance is greater than 0, paid=false. If a value is unreadable use null. Do not infer a customer number unless it is explicitly visible. If only part of the customer name is legible, keep that partial text but add a warning that the name is partial. Do not create daily revenue from an individual ticket screenshot. Never include payment card numbers, CVV, PINs, passwords or secrets. Add a warning for important ambiguity. Return one ticket when a ticket window is clearly visible.`},
