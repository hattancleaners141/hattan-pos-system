...
(function () {
  ...
  // V23.2: Full-mode counter uses its own AI card. Replace that card after every render
  // so the typed Enter workflow is visible in Full mode as well as Simple mode.
  function v23FullAiCardHTML() {
    v23EnsureTypedState();
    return `<div class="v8-ai-head v23-ai-head">
      <div><h3 style="margin:0">${icon('sparkle',17)} AI Intake · Voice or Type</h3><div class="v23-ticket-chip">Typing into Ticket ${esc(String(counterDraft.v23TypedTicketSeq || 1))}</div></div>
      <button type="button" id="v3-mic-btn" class="v3-mic-btn ${counterDraft.aiListening?'listening':''}" onclick="posToggleAiVoice()" ${posVoiceSupported()?'':'disabled'}>${icon('mic',19)}</button>
    </div>
    <div class="v2-note" id="v3-mic-status" style="margin-bottom:7px">Type a line and press Enter. Enter keeps adding to the same ticket. Type “separate ticket” to start another ticket. “Today” = RUSH; “tomorrow” and “next week Thursday” set the due date automatically.</div>
    <textarea id="v23-ai-entry" rows="2" onkeydown="v23AiEntryKeydown(event)" placeholder="3 white blouses · Enter\n2 green jackets · Enter\nseparate ticket 3 shirt on hanger no starch · Enter"></textarea>
    <div class="v23-ai-actions"><button class="btn btn-primary btn-sm" onclick="v23TypedIntakeSubmit()">${icon('plus',14)} Add Typed Line</button><button class="btn btn-secondary btn-sm" onclick="v3VoiceParse()">${icon('sparkle',14)} Interpret Voice</button></div>
    <textarea id="v3-ai-transcript" rows="2" oninput="counterDraft.aiTranscript=this.value" placeholder="Voice transcript appears here…">${esc(counterDraft.aiTranscript||'')}</textarea>
    ${v10AiReviewHTML()}`;
  }

  function v23EnhanceFullCounter(content) {
    const card = content?.querySelector?.('.v8-ai-card');
    if (card) { card.innerHTML = v23FullAiCardHTML(); card.classList.add('v23-ai-card'); }
  }

  const v23BaseRenderPosCounter = window.renderPosCounter;
  window.renderPosCounter = function v23RenderPosCounter(content) {
    v23BaseRenderPosCounter(content);
    v23EnhanceFullCounter(content);
  };
  try { renderPosCounter = window.renderPosCounter; } catch (_error) {}
...
(function () {
  ...
  // Make the live shell visibly identify the deployed build instead of inheriting V19 branding.
  try {
    const v23BasePosShellHTML = window.posShellHTML;
    if (typeof v23BasePosShellHTML === 'function') {
      window.posShellHTML = function v23PosShellHTML() {
        return v23BasePosShellHTML().replace(/Staff POS(?: · V[\w. ]+)?/g, 'Staff POS · V23');
      };
      posShellHTML = window.posShellHTML;
    }
  } catch (_error) {}


  function v23ForceBranding(){
    document.querySelectorAll('.ps-brand .wordmark small').forEach(el => { el.textContent = 'Staff POS · V23'; });
  }
  window.v23ForceBranding = v23ForceBranding;
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', v23ForceBranding); else v23ForceBranding();
  try { new MutationObserver(v23ForceBranding).observe(document.documentElement, {childList:true, subtree:true}); } catch (_e) {}

  window.V23_BUILD = '23.3.0';
})();

/* ---------------------- V23.4 BOOT / LOGIN RECOVERY ----------------------
   Older V17/V18/V20 layers each manage the same startup mask. On some reloads
   an inner layer can remove the visible boot card while v20-booting still hides
   the POS shell. Keep a single visible recovery gate until the app is actually
   rendered, then release every legacy boot class together. */
(function v234InstallBootRecovery(){
  const BOOT_CLASSES = ['v20-booting','v18-booting','v17-booting'];
  let released = false;

  function rootHasUi(){
    const root = document.getElementById('pos-root');
    if (!root) return false;
    return !!root.querySelector('.pos-shell, .pos-login-wrap, .v16-setup-gate, .v18-loading-shell, [data-pos-page], .counter-choice');
  }

  function ensureRecoveryScreen(){
    if (released || rootHasUi()) return;
    if (document.getElementById('v17-boot-screen')) return;
    const gate = document.createElement('div');
    gate.id = 'v17-boot-screen';
    gate.className = 'v17-boot-screen';
    gate.setAttribute('role','status');
    gate.setAttribute('aria-live','polite');
    gate.innerHTML = '<div class="v17-boot-card"><div class="logo-mark"></div><strong>Hattan Cleaners</strong><span id="v18-boot-message">Restoring your secure POS session…</span><button id="v18-boot-retry" class="v18-boot-retry" type="button" onclick="location.reload()" hidden>Retry connection</button></div>';
    document.body.appendChild(gate);
  }

  function releaseIfReady(){
    if (released || !rootHasUi()) return false;
    released = true;
  ...
  const observer = new MutationObserver(() => {
    if (!releaseIfReady()) ensureRecoveryScreen();
  });

  window.addEventListener('DOMContentLoaded', () => {
    ensureRecoveryScreen();
    const root = document.getElementById('pos-root');
    ...
    window.setTimeout(async () => {
      ...
      if (message) message.textContent = 'Finishing secure sign-in…';
      try {
        if (typeof v16Api === 'function') {
          const session = await Promise.race([
            v16Api('session'),
            new Promise(resolve => window.setTimeout(() => resolve({ok:false, timeout:true}), 5000))
          ]);
          if (session?.ok && session.data?.authenticated) {
            try {
              if (typeof v16SetSession === 'function') v16SetSession(session.data.staff);
              if (typeof v16PullState === 'function') await Promise.race([v16PullState(false), new Promise(resolve => window.setTimeout(resolve, 4000))]);
              if (typeof renderPosRoot === 'function') renderPosRoot();
            } catch (_) {}
          } else if (session && !session.timeout) {
            try {
              if (typeof v16Live !== 'undefined') v16Live.authenticated = false;
              if (typeof renderPosRoot === 'function') renderPosRoot();
            } catch (_) {}
          }
        }
      ...
      if (!releaseIfReady()) {
        if (message) message.textContent = 'The secure POS is taking longer than expected.';
