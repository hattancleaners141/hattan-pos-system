...
function v19EnsureDraft(draft) {
  ...
  if (!Number.isFinite(Number(draft.storeCreditAmount))) draft.storeCreditAmount = 0;
...
posCompleteDropOff = function v19CompleteDropOff() {
  ...
  v19DropOffCreditContext = {
    ...
    remaining:Math.min(Number(customer?.storeCredit || 0), Math.max(0, Number(draft.storeCreditAmount || customer?.storeCredit || 0))),
    applied:0,
  };
