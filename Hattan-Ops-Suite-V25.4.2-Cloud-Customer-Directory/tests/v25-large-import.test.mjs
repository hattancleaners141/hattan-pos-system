...
test('large import card exposes the final import action', () => {
  assert.match(client, /Final Import to Hattan Cloud/);
  assert.match(client, /onclick="v25Commit\(\)"/);
  assert.match(client, /Import Completed/);
});
...
test('large imported customers are searched remotely and cached on demand', () => {
  ...
  assert.match(search, /limit=\$\{limit \+ 1\}/);
  assert.match(search, /requireSession\(event\)/);
  assert.match(html, /v25-large-import\.js/);
});

test('customer directory loads cloud customers in pages and supports more results', () => {
  assert.match(client, /remoteDirectorySearch/);
  assert.match(client, /v25LoadMoreCustomers/);
  assert.match(client, /Loading imported customers/);
  assert.match(search, /page \* limit/);
  assert.match(search, /hasMore/);
  assert.match(html, /v25-cloud-customers\.css/);
});
