...
export const handler = async event => {
  ...
  try {
    ...
    const page = Math.max(0, Math.min(10000, Number(event.queryStringParameters?.page || 0)));
    const limit = 100, offset = page * limit;
    const pattern = encodeURIComponent(`*${q}*`);
    const search = q ? `&or=(legacy_customer_number.ilike.${pattern},name.ilike.${pattern},phone.ilike.${pattern},email.ilike.${pattern},address_line1.ilike.${pattern},postal_code.ilike.${pattern})` : '';
    const rows = await selectRows('legacy_customers', `store_id=eq.${encodeURIComponent(storeId())}${search}&order=name.asc.nullslast,legacy_customer_number.asc&limit=${limit + 1}&offset=${offset}`, 'legacy_customer_number,name,phone,email,address_line1,apartment,city,state,postal_code,balance,store_credit,memo,preferences,default_fulfillment,last_visit,join_date,route,customer_type');
    const hasMore=(rows||[]).length>limit;
    return json(200,{ok:true,customers:(rows||[]).slice(0,limit).map(posCustomer),page,hasMore,query:q});
