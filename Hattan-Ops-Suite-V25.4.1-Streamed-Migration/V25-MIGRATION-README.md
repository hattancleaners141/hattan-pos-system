# Hattan Ops Suite V25.4 Migration Center

## Required one-time Supabase setup

Before the first V25.4 large import, open Supabase → SQL Editor and run the complete updated `supabase/schema.sql` included in this package. It creates the normalized `legacy_customers` and `migration_imports` tables plus the secure 500-row upsert function. The script is idempotent and can be run against the existing Hattan project.

Do not press Final Import until this SQL has completed successfully. If the database setup is missing, V25.4 stops the import and shows an error; it does not fall back to the oversized POS-state snapshot.

## Large-file behavior

- Large Visual Cleaner customer reports and `vcust.dbf` are queued without placing all records in browser storage.
- Final Import sends authenticated batches of at most 500 customers.
- The database upserts on `(store_id, legacy_customer_number)` and retains existing fields whenever a newer source has a blank value.
- SHA-256 file fingerprints skip an already-completed exact file.
- A changed/newer export has a different fingerprint and updates the matching customer numbers while adding genuinely new numbers.
- Imported customers are searched from Supabase on demand and only a small working cache is held in the browser.
- Interrupted imports can be reselected and restarted safely; completed batches are idempotent.
